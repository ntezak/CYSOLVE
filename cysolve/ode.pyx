...
from cython_gsl cimport (
    ...
    gsl_rng_env_setup, gsl_rng, gsl_rng_type, gsl_rng_free, gsl_rng_alloc, gsl_rng_default, gsl_ran_gaussian,
...
cdef int c_integrate_sde(double tf, double h, Py_ssize_t N, Py_ssize_t dim, Py_ssize_t m, double * y0,  double * Y, ode_t ode, ode_t noise_coeff, int noise_coeff_is_covariance, void *params, double t0=0) nogil:
    """
    ...
    for kk in range(N):
        ...
        for jj in range(stepspersample):
            ...
            # 1) compute noise-scaling matrix
            status = noise_coeff(t, y, B.data, params)
            
            if status != GSL_SUCCESS:
                return 201
            
            # 2) multiply noise_coeff * wiener increments to get noises
            
            # if noise_coeff is actually the covariance matrix, 
            # we need to find a square root (via Cholesky decomp) 
            # and apply that.

            if noise_coeff_is_covariance:
                # 3) sample gaussians
                sample_gaussian(r, sigma, noises.data, m)
                status = gsl_linalg_cholesky_decomp(B)
                if status != GSL_SUCCESS:
                    return status
                gsl_blas_dtrmv(CblasLower, CblasNoTrans, CblasNonUnit, B, noises)
            else:
                sample_gaussian(r, sigma, w.data, m)
                gsl_blas_dgemv(CblasNoTrans, 1.0, B, w, 0.0, noises)
...
cdef class SDEs(ODEs):
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cpdef object integrate_sde(self, double delta_t, double h, Py_ssize_t N, int include_initial=0, int update_state=1):
    # cpdef object integrate(self, double delta_t, Py_ssize_t N, int include_initial=0, int update_state=1):
        """
        Starting from the current states self.y and at the current time self.t integrate for a duration delta_t and
        evaluate the states at N time intervals. 
        ...
        cdef:
            ...
            np.ndarray success = np.zeros(self.N_params, dtype=int)
            long[:] successview = success
            int noise_coeff_is_covariance = self.noise_coeff_is_covariance
            
        c_ode = self.c_ode
        c_noise_coeff = self.c_noise_coeff
            
        for traj_index in prange(self.N_params, nogil=True):
            successview[traj_index] = c_integrate_sde(t + delta_t, h, N, dim, m, <double*>&yview[traj_index,0], <double*>&YView[traj_index * strd], c_ode, c_noise_coeff, noise_coeff_is_covariance, c_params + param_size * traj_index, t)
